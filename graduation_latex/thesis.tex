...
%%%%%%%%%%%%%%%%%%%%%%%%%先验缺失的抓取点自主生成研究%%%%%%%%%%%%%%%%%%
\chapter[先验信息缺失的抓取点自主生成]{先验信息缺失的抓取点自主生成}
\section{引言}[Content specification]
...
\begin{figure}[h]
	...
	\subfigure[两点云分布（初始）]{
		\begin{minipage}[h]{0.4\textwidth}
			\includegraphics[width=1\textwidth]{chapter3/ICP示意图0迭代}
		\end{minipage}
		\label{两点云分布（初始）}
	}
    	\subfigure[两点云分布（迭代20次）]{
    		\begin{minipage}[h]{0.4\textwidth}
   		 	\includegraphics[width=1\textwidth]{chapter3/ICP示意图20迭代}
    		\end{minipage}
		\label{两点云分布（迭代20次）}
    	}
	\caption{ICP算法效果展示}
...
以猴子面部的点云模型为例，初始情况下将点云模型略微调整位姿，使用ICP算法不断迭代以求得两点云的变换矩阵。通过20次迭代，基本可以实现两点云重合。迭代前后点云位姿如图\ref{}所示，其中红色点云为正在迭代的点云分布，白色点云为猴子面部模型初始点云分布。
\subsection{算法实现}
...
\begin{figure}[h]
\centering
\includegraphics[width = 0.6\textwidth]{chapter3/基于模型的点云识别实操图}
...
\begin{figure}[h]
\centering
\includegraphics[width = 0.6\textwidth]{chapter3/基于模型的点云识别效果图}
...
这里特别解释一下图\ref{GG-CNN输出（初始）}中图Q表示抓取质量热度图，颜色越偏暖色越值得被抓，图Angle表示抓取角度图，单位是弧度，可以看到对柱形物体，抓取角度在高抓取质量点处几乎一致。另外，这里给出网络框架优化前后的训练过程折线图，优化后的网络（红线）将能更快的收敛并达到更高的预测准确率。
\begin{figure}[h]
\centering
\includegraphics[width = 1.0\textwidth]{chapter3/训练对比图}
\caption{优化前后预测准确率对比折线图}
\label{训练对比图}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width = 0.93\textwidth]{chapter4/图像处理流程图}
\caption{图像处理流程图}
\label{图像处理流程图}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width = 0.55\textwidth]{chapter4/俯视相机成图}
\caption{俯视相机成图}
\label{俯视相机成图}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width = 0.64\textwidth]{chapter4/目标特征生成}
\caption{目标特征生成}
\label{目标特征生成图}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width = 0.64\textwidth]{chapter4/指令发散}
\caption{指令发散现象}
\label{指令发散现象}
\end{figure}
...
在运行IBVS程序，时遇到了Z轴速度指令一直接近0的问题，不论抓取对象是笔袋还是卷纸。指令曲线如图\ref{Z轴速度接近0的情况}所示。经过反复实验发现只要特征分布恰好全在现实世界的一个平面上且旋转角度偏差很小的时候就会导致这个问题。对于笔袋，当前设定的特征分布相对聚集，所有特征点都在笔袋上；对于卷纸，当前设定的特征分布相对松散，所有特征点都在平台上，所以需要优化当前特征分布。\begin{figure}[h]
\centering
...
\begin{figure}[h]
	...
	\subfigure[速度指令]{
		\begin{minipage}[h]{0.40\textwidth}
			\includegraphics[width=1\textwidth]{chapter5/PD控制器/speed_command0} 
		\end{minipage}
		\label{视觉伺服曲线（PD）a}
	}
    	\subfigure[特征偏差平方和]{
	...
	\subfigure[X方向特征偏差]{
		\begin{minipage}[h]{0.40\textwidth}
			\includegraphics[width=1\textwidth]{chapter5/PD控制器/error_x0} 
		\end{minipage}
		\label{视觉伺服曲线（PD）c}
	}
    	\subfigure[Y方向特征偏差]{
    		\begin{minipage}[h]{0.40\textwidth}
		 	\includegraphics[width=1\textwidth]{chapter5/PD控制器/error_y0}
    		\end{minipage}
		\label{视觉伺服曲线（PD）d}
    	}
	\caption{视觉伺服曲线（PD）}
...
\begin{figure}[h]
	...
	\subfigure[特征分布（正常）]{
		\begin{minipage}[h]{0.4\textwidth}
			\includegraphics[width=1\textwidth]{chapter5/特征分布（正常）}
		\end{minipage}
		\label{自定义特征点分布（正常）}
	}
	\subfigure[特征分布（波动）]{
		\begin{minipage}[h]{0.4\textwidth}
			\includegraphics[width=1\textwidth]{chapter5/特征分布（波动）}
		\end{minipage}
		\label{自定义特征点分布（波动）}
	}
...
\begin{figure}[h]
	...
	\subfigure[速度指令]{
		\begin{minipage}[h]{0.4\textwidth}
			\includegraphics[width=1\textwidth]{chapter5/滑模控制器/speed_command0} 
		\end{minipage}
		\label{视觉伺服曲线（滑模控制）a}
	}
    	\subfigure[特征偏差平方和]{
	...
	\subfigure[X方向特征偏差]{
		\begin{minipage}[h]{0.4\textwidth}
			\includegraphics[width=1\textwidth]{chapter5/滑模控制器/error_x0} 
		\end{minipage}
		\label{视觉伺服曲线（滑模控制）c}
	}
    	\subfigure[Y方向特征偏差]{
    		\begin{minipage}[h]{0.4\textwidth}
		 	\includegraphics[width=1\textwidth]{chapter5/滑模控制器/error_y0}
    		\end{minipage}
		\label{视觉伺服曲线（滑模控制）d}
    	}
	\caption{视觉伺服曲线（滑模控制）}
...
\begin{figure}[h]
	...
	\subfigure[速度指令]{
		\begin{minipage}[h]{0.40\textwidth}
			\includegraphics[width=1\textwidth]{chapter5/自适应控制器/speed_command0} 
		\end{minipage}
		\label{视觉伺服曲线（自适应）a}
	}
    	\subfigure[特征偏差平方和]{
	...
	\subfigure[X方向特征偏差]{
		\begin{minipage}[h]{0.40\textwidth}
			\includegraphics[width=1\textwidth]{chapter5/自适应控制器/error_x0} 
		\end{minipage}
		\label{视觉伺服曲线（自适应）c}
	}
    	\subfigure[Y方向特征偏差]{
    		\begin{minipage}[h]{0.40\textwidth}
		 	\includegraphics[width=1\textwidth]{chapter5/自适应控制器/error_y0}
    		\end{minipage}
		\label{视觉伺服曲线（自适应）d}
    	}
	\caption{视觉伺服曲线（自适应）}
...
\begin{figure}[h]
	...
	\subfigure[速度指令]{
		\begin{minipage}[h]{0.40\textwidth}
			\includegraphics[width=1\textwidth]{chapter5/特征偏差收敛一致化/speed_command0} 
		\end{minipage}
		\label{视觉伺服曲线（最终效果）a}
	}
    	\subfigure[特征偏差平方和]{
	...
	\subfigure[X方向特征偏差]{
		\begin{minipage}[h]{0.40\textwidth}
			\includegraphics[width=1\textwidth]{chapter5/特征偏差收敛一致化/error_x0} 
		\end{minipage}
		\label{视觉伺服曲线（最终效果）c}
	}
    	\subfigure[Y方向特征偏差]{
    		\begin{minipage}[h]{0.40\textwidth}
		 	\includegraphics[width=1\textwidth]{chapter5/特征偏差收敛一致化/error_y0}
    		\end{minipage}
		\label{视觉伺服曲线（最终效果）d}
    	}
	\caption{视觉伺服曲线（最终效果）}
...
\begin{figure}[h]
	\centering
	\subfigure[X方向特征偏差（处理前）]{
		\begin{minipage}[h]{0.35\textwidth}
			\includegraphics[width=1\textwidth]{chapter5/error_x0（处理前）}
		\end{minipage}
		\label{X方向特征偏差（处理前）}
	}
	\hspace{0.05\textwidth}
	\subfigure[X方向特征偏差（处理后）]{
		\begin{minipage}[h]{0.35\textwidth}
...
根据特征偏差收敛速度一致性处理后，对X方向特征偏差放大细节并比较处理前的曲线，如图所示\ref{X方向特征偏差细节放大比较}，可以很明显看出处理后各个特征收敛曲线变得一致起来。
...
\begin{figure}[h]
	...
	\subfigure[超调量比较]{
		\begin{minipage}[h]{0.45\textwidth}
			\includegraphics[width=1\textwidth]{chapter5/超调量比较}
		\end{minipage}
		\label{超调量比较}
	}
	\subfigure[调节时间比较]{
		\begin{minipage}[h]{0.45\textwidth}
			\includegraphics[width=1\textwidth]{chapter5/调节时间比较}
		\end{minipage}
		\label{调节时间比较}
	}
...
就此已经完成了未知物体抓取算法中所有的研究内容，为了明确表达出整个系统的样子，最终系统控制示意框图确认为图\ref{控制示意框图}所示。系统通过图像处理和GG-CNN自主生成目标特征作为控制期望，比较当前提取出的基于抓取点的自定义特征，经过视觉伺服优化后的控制律，对整个系统下达末端速度指令。通过各关节逆运动解算对机械臂进行控制。由于程序中的类错综复杂，所以以ROS的节点流形式将程序绘运作绘制成图\ref{控制示意框图}所示，旨在形象表达程序的框架和运作方式。
\begin{figure}[h]
	\centering
...
\begin{figure}[h]
	\centering
	\includegraphics[width=1.0\textwidth]{chapter5/程序节点数据流图}
	\caption{程序节点数据流图}
	\label{程序节点数据流图}
\end{figure}
...
\begin{figure}[h]
	...
	\subfigure[硬件实物图]{
		\begin{minipage}[h]{0.43\textwidth}
			\includegraphics[width=1\textwidth]{chapter6/硬件实物图}
		\end{minipage}
		\label{硬件实物图}
	}
	\subfigure[抓取目标选取]{
		\begin{minipage}[h]{0.47\textwidth}
			\includegraphics[width=1\textwidth]{chapter6/抓取目标选取}
		\end{minipage}
		\label{抓取目标选取}
	}
	\caption{实验准备}
...
\begin{figure}[h]
	...
	\subfigure[X方向速度]{
		\begin{minipage}[h]{0.40\textwidth}
			\includegraphics[width=1\textwidth]{chapter6/重复性测试X} 
		\end{minipage}
		\label{重复性精度测试a}
	}
    	\subfigure[Y方向速度]{
    		\begin{minipage}[h]{0.40\textwidth}
   		 	\includegraphics[width=1\textwidth]{chapter6/重复性测试Y}
    		\end{minipage}
		\label{重复性精度测试b}
    	}
	\\ 
	\subfigure[Z方向速度]{
		\begin{minipage}[h]{0.40\textwidth}
			\includegraphics[width=1\textwidth]{chapter6/重复性测试Z} 
		\end{minipage}
		\label{重复性精度测试c}
	}
    	\subfigure[Z轴角速度]{
    		\begin{minipage}[h]{0.40\textwidth}
		 	\includegraphics[width=1\textwidth]{chapter6/重复性测试ZZ}
    		\end{minipage}
		\label{重复性精度测试d}
    	}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.45\textwidth]{chapter6/重复性精度测试定位测试}
	\caption{重复性精度测试定位测试}
	\label{重复性精度测试定位测试}
\end{figure}
...
\begin{figure}[h]
	...
	\subfigure[相同目标不同位姿]{
		\begin{minipage}[h]{0.4\textwidth}
			\includegraphics[width=1\textwidth]{chapter6/相同物体不同位姿}
		\end{minipage}
		\label{相同目标不同位姿特征偏差平方和均值}
	}
	\subfigure[相同位姿不同目标]{
		\begin{minipage}[h]{0.4\textwidth}
			\includegraphics[width=1\textwidth]{chapter6/相同位姿不同物体}
		\end{minipage}
		\label{超调响应时间直方图1}
	}
	\caption{特征偏差平方和}
...
\begin{figure}[h]
	...
	\subfigure[相同物体不同位姿]{
		\begin{minipage}[h]{0.80\textwidth}
			\includegraphics[width=1\textwidth]{chapter6/相同物体不同位姿（条形）} 
		\end{minipage}
		\label{相同物体不同位姿伺服性能}
	}
	\\ 
	\subfigure[相同位姿不同物体]{
		\begin{minipage}[h]{0.80\textwidth}
			\includegraphics[width=1\textwidth]{chapter6/相同位姿不同物体（条形）} 
		\end{minipage}
		\label{相同位姿不同物体伺服性能}
	}
